...
@Component({
  selector: 'app-seat-booking-dialog',
  templateUrl: './seat-booking-dialog.component.html',
  styleUrls: ['./seat-booking-dialog.component.css']
})
export class SeatBookingDialogComponent implements OnInit {
  selectedDate = new FormControl(new Date());
  formGroup!: FormGroup;
  actionBtn: string = 'BOOK NEW SEAT';
  branchList: any;
  isActiveId: any;
  branchId: any;
  timeSlotList: any;
  ...
  formData: any;
  rows: any = [];
  seatsPerRow = 12; // adjust to your needs
  options: any;
  finaldata: any;
  pendingCount: any;
  ...
  constructor(private sharedService: SharedService, private formBuilder: FormBuilder, private router: Router, public dialog: MatDialog, private alertify: AlertifyService, private service: OccupiedSeatService, @Inject(MAT_DIALOG_DATA) public editData: any, private dialogRef: MatDialogRef<SeatBookingDialogComponent>) {
  ...
  getAvilabeSeats() {
    ...
    if(value == true){
      if (this.formGroup.valid) {
        ...
        this.service.postAllAvilableSeat(this.formData)
          .subscribe({
            next: (res) => {
              this.clearArray();
              if (res.data.length > 0) {
                let seatList = res.data.filter((item: any) => item.isActive == 'Active')
                for (let i = 0; i < seatList.length; i += this.seatsPerRow) {
                  let row = [];
                  for (let j = i; j < i + this.seatsPerRow; j++) {
                    if (j < seatList.length) {
                      row.push(seatList[j]);
                    }
                  }
                  this.rows.push(row);
                ...
                this.alertify.alert("SEAT DETAILS", res.message);
              }
            },
            error: (res) => {
  ...
  openBookingDialog(row: any) {
    ...
    this.dialog.open(BookingDialogComponent, {
      ...
      if (val === 'SAVE') {
        ...
        this.sharedService.triggerGetSeatBooking();
      }
    })
  }
}