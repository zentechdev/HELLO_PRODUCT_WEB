...
@Component({
  selector: 'app-visiting-card-access-dialog',
  templateUrl: './visiting-card-access-dialog.component.html',
  styleUrls: ['./visiting-card-access-dialog.component.css']
})
export class VisitingCardAccessDialogComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private alertify: AlertifyService, private service:VisitingCardAccessService, @Inject(MAT_DIALOG_DATA) public editData: any, private dialogRef: MatDialogRef<VisitingCardAccessDialogComponent>) { this.dialogRef.disableClose = true }
  ...
  ngOnInit(): void {
    ...
    if (this.editData) {
      ...
      this.formGroup.controls['isActive'].setValue(this.editData.visitingCardStatus);
    }
  ...
  putData() {
    ...
    if (this.formGroup.valid) {
      this.service.updateVisitingCardStatus(formGroup, this.editData.employeeCode)
        .subscribe({
          next: (res) => {
